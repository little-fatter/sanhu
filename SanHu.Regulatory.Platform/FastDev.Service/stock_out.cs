...
{
    public class stock_out : ServiceBase, IService
    {
        public stock_out()
        {
            OnSave += stock_out_OnSave;

            OnBeforeSave += stock_out_OnBeforeSave;


            OnDelete += stock_out_OnDelete;
        }



        void stock_out_OnSave(object entity, object viewdata, bool isCreate)
        {
            var db = this.QueryDb;

            var viewData = viewdata as Model.Form.stock_out;
            ...
            if (!isCreate)
            {
                var status = db.ExecuteScalar<string>("select Status from stock_out where ID = @0", viewData.ID);
        ...
        void stock_out_OnBeforeSave(object entityObj, object viewdata, bool isInsert)
        {
            var db = this.QueryDb;
            var sn = GetSN(db, "stock_pileDetail", "SerialNumber");
            var entity = entityObj as Model.Entity.stock_out;
            var viewData = viewdata as Model.Form.stock_out;
            ...
            var details = db.Fetch<Model.Entity.stock_outdetails>("where OrderID = @0", id);
            foreach (var detail in details)
            {
                detailIndex++;
                ...
                piledetail.DocumentType = entity.OutType == "other" ? "order_out" : "check_out";
        ...
        void stock_out_OnDelete(object[] args)
        {
            ...
            {
                var entity = db.FirstOrDefault<Model.Entity.stock_out>("where ID = @0", id);
                var userid = entity.CreateUserID;
