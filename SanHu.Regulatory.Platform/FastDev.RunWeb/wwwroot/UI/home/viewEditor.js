...
function view()
{
    return function run(renderTo, pagedata)
    {
        ...
        var width1 = 180, width2 = 480;
        var view_width_diff = - 80;
        var jtarget1 = $('<ul></ul>'),
            jtarget2 = $('<div></div>'),
            jtarget3 = $('<div class="ne-view"></div>');
        var codeMirror;
        var code_view = "", code_service = "", currentIsSerivce = false, currentView = null;

        jtarget1.height( h ).css( "overflow", "auto" );
        jtarget2.height(h);
        ...
        { 
            ...
            var tree = jtarget1.ligerTree({
                ...
                isExpand: function (e)
                {
                    ...
                    if (data.children && data.type=="model")
                    {
                        if ( arg.indexOf( data.id ) > -1 ) //展开
                        {
                            for ( var i = 0; i < data.children.length; i++ )
                            {
                                if ( arg.indexOf( data.children[i].id ) > -1 )
                                { 
                                    tree.selectNode( data.children[i], true );
                                    return true;
                                }
                            }
                            return true;
        ...
        function init_view()
        {
            $(renderTo).find(".l-panel:eq(2)")
                .css("background", "white")
                .find(".l-panel-header").hide();
        ...
        function show_view()
        {
            ...
            var od = jtarget3.width(), oh = jtarget3.height();
