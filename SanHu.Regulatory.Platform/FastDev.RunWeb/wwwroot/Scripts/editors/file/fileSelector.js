(function ($)
{
    ...
    $.ligerDefaults.Form.editors['fileSelector'] = {
        create: function (container, editParm,p)
        {
            ...
            var form = this; 
            var jtext = $('<div class="l-text file-text combobox-selector"><div class="l-trigger" title="浏览"><div class="l-trigger-icon" ></div></div></div>').appendTo(container);
            var jhide = $('<input type="hidden" class="filevalue"  />  ').appendTo(jtext);
            ...
            jtext.find(".l-trigger").click(function ()
            {
                    ...
                    buttons: [
                        {
                            ...
                            { 
                                ...
                                jtext.find(".filetag").remove();
                                jtext.removeClass("hasfile");
                                if (!list || !list.length)
                                { 
                                    jhide.val(''); 
                                } else
                                {
                                    ...
                                    { 
                                        ...
                                        jtext.addClass("hasfile");
                                        var jfiletag = $('<div class="filetag"></div>').appendTo(jtext); 
                                        createLink(this.url).text(this.title).appendTo(jfiletag);
                                        $('<a class="btnremove" href="javascript:void(0)">x</a>').appendTo(jfiletag).click(function ()
                                        {
                                            var newurls = [];
                                            jfiletag.parent().find(".filetag").not(jfiletag).each(function ()
                                            {
                                                newurls.push($(this).find(".filetaglink").attr("data-url"));
                                            });
                                            jhide.val(newurls.join(';'));
        ...
        setValue: function (editor, value, editParm)
        {
        	...
        	var isReadonly = field.readonly || (field.editor && field.editor.readonly);
            if (value)
            { 
                editor.find(".filevalue").val(value);

                var urls = value.split(';');
                $(urls).each(function ()
                {
                    var fileName = getFileName(this); 
                    editor.addClass("hasfile");
                    var jfiletag = $('<div class="filetag"></div>').appendTo(editor);
                    createLink(this).text(fileName).appendTo(jfiletag);
                    $('<a class="btnremove" href="javascript:void(0)">x</a>').appendTo(jfiletag).click(function ()
                    {
                    	if (isReadonly) return;
                        var newurls = [];
                        ...
                        editor.find(".filevalue").val(newurls.join(';')); 
                        jfiletag.remove();
                        if (!newurls.length)
                        {
                            editor.removeClass("hasfile");
                        }
    ...
    function createLink(url)
    {
        ...
        }).attr("data-url", url).attr("title", url);
        return jlink;
    }
    ...
    var CONFIG = {
        ...
        "fileManagerListSize": 200, /* 每次列出文件数量 */
    ...
    pbc.editors.fileSelector = fileSelector

    $.extend(fileSelector.prototype, {
        render: function ()
        {
