...
(function ($)
{
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            {
                g.menu = g.tab.menu = $.ligerMenu({ width: 100, items: [
        ...
        addTabItem: function (options)
        {
            ...
            else if (options.target)
            {
                ...
                contentitem.addClass("l-tab-content-item-target");
                
                options.target.tabOpener = g;
                if (options.data)
                {
                    options.target.openerData = options.data;
                }
                if (options.callback)
                    options.callback();
            }
        ...
        reload: function (tabid)
        {
            ...
            { 
                var jtarget = contentitem.children().get(0);
                if ($.isFunction(jtarget.tabReload))
                {
                    jtarget.tabReload.call(g, {
                        target: jtarget,
                        tabid: tabid 
                    });
                }
