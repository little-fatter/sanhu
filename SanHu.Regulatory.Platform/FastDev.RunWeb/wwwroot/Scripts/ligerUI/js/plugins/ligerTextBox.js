...
(function ($)
{
    ...
    $.ligerDefaults.TextBox = {
        ...
        initSelect : false,
        value: null,     //初始化值 
        valueRegex: null,   //基于对正则表达式的限制
        valueCheck: null,   //基于对自定义函数的限制,返回false表示不通过
    ...
    $.ligerui.controls.TextBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _setEvent: function ()
        {
            ...
            if (p.valueRegex)
            {
                ...
                {
                    var value = g.inputText.val();
                    if (!p.valueRegex.test(value))
                    {
                        g.inputText.val(g.inputText.attr("data-oldvalue"));//恢复值
                    }
                }
                g.inputText.keyup(checkReg).bind("paste", checkReg);
            }
            if (p.valueCheck)
            {
                function checkFn()
                { 
                    var value = g.inputText.val(), result = true;
                    if (typeof (p.valueCheck) == "string")
                    {
                        result = (function ()
                        {
                            try
                            {
                                var row = g.get('host_grid_row');
                                var fn = null;
                                var evalStr = "fn = function(value,row){ " + (p.expressionInit || "") + " return " + p.valueCheck + "}"; 
                                eval(evalStr);
                                return fn(value, row);
                            }
                            catch (e)
                    ...
                    {
                        result = p.valueCheck(value);
                    }
                    if (result === false)
                    {
                        var resetValue = p.valueReset ? p.valueReset(value) : null;
                        if (!resetValue && resetValue !== "0" && resetValue !== 0)
                        {
                            resetValue = g.inputText.attr("data-oldvalue");
                        }
                        g.inputText.val(resetValue);//恢复值
                    }
                }
                g.inputText.change(checkFn);
            }
            
            else if (p.digits || p.number || p.currency)
            { 
                g.inputText.keyup(keyCheck).bind("paste", keyCheck); 
            } 
            g.inputText.bind('blur.textBox', function ()
            {
                g.trigger('blur', g.inputText.val());
                g.checkNotNull();
                g.formatValue();
        ...
        _setValue: function (value)
        {
            if (value != null)
            {
                ...
                this.inputText.attr("data-oldvalue", value);
            }
            this.checkNotNull();
        },
