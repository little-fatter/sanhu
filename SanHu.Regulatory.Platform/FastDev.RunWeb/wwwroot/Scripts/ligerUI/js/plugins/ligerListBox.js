...
(function ($)
{
    ...
    $.ligerui.controls.ListBox.ligerExtend($.ligerui.controls.Input, {
        ...
        refresh:function()
        {
            ...
            this.clearContent();
            if (!data) return;
            var valueIndexStr = " value='ALL' index='-1'";
            if (p.columns)
            {
                g.selectBox.table.headrow = $("<tr class='l-table-headerow'><td width='18px' class='l-checkboxrow'></td></tr>");

                if (p.isShowSelectAll)
                {
                    g.selectBox.table.headrow.find("td").append("<input class='chkall' type='checkbox'" + valueIndexStr + "/>");
                }
            ...
            if (p.isShowSelectAll && !p.columns)
            {
               
                out.push("<tr>");
                out.push("<td style='width:18px;' class='l-checkboxrow'><input class='chkall' type='checkbox'" + valueIndexStr + "/></td>");
                out.push("<td align='left'>(全部)</td></tr>");
            }
            ...
            g.selectBox.table.find("input.chkall").change(function ()
            {
                ...
                if (!isChecked)
                {
                    $("tr.l-selected", g.selectBox).removeClass("l-selected")
                } else
                {
                    $("tr", g.selectBox).addClass("l-selected")
                }

                g.selectBox.table.find(":checkbox").each(function () { this.checked = isChecked; });

                g._updateValue();

                g.trigger('selected', ['ALL']);
            } );

            if ( p.readonly )
            {
                $( g.element ).find( ":checkbox" ).bind( 'click', function () { return false; } );
            }
        },
        _getValue: function ()
        {
        ...
        isAllSelected: function ()
        {
            var g = this, p = this.options;
            var selected = true;
            $("td > :checkbox", g.selectBox).each(function ()
            {
                if ($(this).hasClass("chkall")) return;
                if (!this.checked) selected = false;
            });
            return selected;
        },
        selectAll: function (key)
        {
            ...
            $("tr", g.selectBox).each(function ()
            {
                ...
                if (value != undefined)
                {
                    if (key && value.toString().indexOf(key) == -1)
                    {
                        jrow.removeClass("l-selected").find(":checkbox").each(function () { this.checked = false; });
                        return; //过滤选择
                    }
                    values.push(jrow.attr("value"));

                    jrow.addClass("l-selected").find(":checkbox").each(function () { this.checked = true; });
                }
            }); 
            g._updateChkAll();
        ...
        _updateChkAll : function(){
            var g = this, p = this.options;
            if (!p.isShowSelectAll) return;
            var isChecked = true, firstRow = null;
            $("tr", g.selectBox).each(function (i)
            {
                if (i == 0)
                {
                    firstRow = $(this);
                ...
                var jrow = $(this);
                if (!jrow.find(":checkbox").get(0).checked) isChecked = false;
            });
            if (isChecked)
            {
                firstRow.addClass("l-selected").find(":checkbox").each(function () { this.checked = true; });
            } else
            {
                firstRow.removeClass("l-selected").find(":checkbox").each(function () { this.checked = false; });
            }
        },
        _dataInit: function ()
        {
        ...
        _addClickEven: function ()
        {
            ...
            g.selectBox.click(function (e)
            {
                ...
                if ($(obj).hasClass("l-listbox-inner") || $(obj).hasClass("l-listbox") || $(obj).hasClass("chkall")) return;
