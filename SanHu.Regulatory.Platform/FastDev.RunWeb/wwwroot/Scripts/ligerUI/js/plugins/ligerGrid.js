...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        mouseoverRowBgColor: null,
        selectedRowBgColor: null ,
        enabledSort: true,                      //是否允许排序
        enabledShiftKey : false,                
        rowClsRender: null,                   //行自定义css class渲染器
        rowAttrRender: null,                  //行自定义属性渲染器(包括style，也可以定义)
        groupValueGetter : null,                //分组 - 获取分组值函数
        ...
        selectableInEdit : false,               //编辑状态是否可选择
        selectable: true,
        ...
        rownumbersByPage : false,             //行序号随着分页改变而改变
        ...
        getContainerHeight : null,          //自定义整体宽度的计算
        scrollToPage: false,               //滚动时分页
        scrollToAppend: true,             //滚动时分页 是否追加分页的形式
        cellSelectable: false,          //是否启用单元格选中状态 
    ...
    $.ligerDefaults.Grid.sorters['number'] = function (val1, val2)
    {
        return parseInt(val1) < parseInt(val2) ? -1 : parseInt(val1) > parseInt(val2) ? 1 : 0;
    }; 
    ...
    $.ligerDefaults.Grid.formatters['number'] = function (value,column)
    {
        ...
        try{
            var r = parseFloat(value).toFixed(column.decimalplace || 2);
            if (r == "NaN") return value;
            return r;
        }
        catch (e)
        {
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _init: function ()
        {
            ...
            if ($.inArray(p.pageSize, p.pageSizeOptions) == -1)
            {
                ...
                p.pageSizeOptions.sort(function (a,b)
                {
                    return a > b
                });
            }
        ...
        _setIsShowFilter: function (value)
        {
            var g = this, p = this.options;
            if (value)
            {
        ...
        //获取承载grid容器的高度(通常是计算window)
        _getContainerHeight: function ()
        {
            var g = this, p = this.options;
            if ($.isFunction(p.getContainerHeight))
            {
                return p.getContainerHeight.call(g);
            } 
        ...
        {
            var g = this, p = this.options;
            if ($.isFunction(p.getContainerOffsetTop))
            {
                return p.getContainerOffsetTop.call(g);
            }
            return 0;
        },
        _calculateGridBodyHeight: function (h)
        {
            ...
            {
                if (p.inWindow)
                    h = g._getContainerHeight() * parseInt(h) * 0.01;
                else
                    h = g.grid.parent().height() * parseInt(h) * 0.01;
            }
            if (p.title) h -= 24;
            if (p.usePager) h -= g.toolbar.outerHeight();
            if (p.totalRender) h -= p.totalHeight || 25;
        ...
        _getRawRow : function(row){
            var g = this, p = this.options;
            if (!g.rawRows) return null;
            for (var i = 0; i < g.rawRows.length; i++)
            {
                if (g.rawRows[i].__row == row) return g.rawRows[i];
            }
            return null;
        },
        _saveRaw: function (data)
        {
            ...
            if (data && data[p.root])
            {
                $(data[p.root]).each(function (i, row)
                {
                    g.gridRows.push(row);
                    var rawRow = $.extend(true, {}, row);
                    rawRow.__row = row;
                    g.rawRows.push(rawRow);
                });
            }
        },
        //刷新数据
        loadData: function (loadDataParm,sourceType)
        {
            ...
            var clause = null;
            var loadServer = true;
            if (loadDataParm == undefined)
            {
                loadDataParm = g.currentFilterFn;
            } 
            ...
            if (p.dataType == "local")
            {
                ...
                //061122 改为从实时数据源(gridRows)读取
                g.filteredData = {};
                g.filteredData[p.root] = [];
                g.sortedData = null; 
                $(g.gridRows).each(function () { g.filteredData[p.root].push(this); });
        ...
        loadServerData: function (param, clause, sourceType)
        {
            ...
            var ajaxOptions = {
                ...
                success: function (data)
                {
                    ...
                    if ( $.isFunction( p.dataGetter ) )
                    {
                        var result = p.dataGetter.call( g, data );
        ...
        */
        ...
        submitEdit: function (rowParm)
        {
            ...
            else
            {
                ...
                var evenResult = g.trigger('beforeSubmitEdit', { record: rowdata, rowindex: rowdata['__index'], newdata: newdata });
                if (evenResult == false)
                    return false;
                if ($.isFunction(evenResult))
                {
                    evenResult(function ()
                    {
        ...
        _enabledEditByCell: function ( cell, name )
        {
            var g = this, p = this.options;
            var column = g.getColumn(cell);
            if ( !column ) return false;
            if ( name && column.name !== name ) return false;
            return column.editor && column.editor.type;
        },
        isEditing: function ()
        {
            var g = this, p = this.options;
            if (g.editor && g.editor.editing) return true;
            return false;
        },
        moveSelectedCell: function (type)
        {
            var g = this, p = this.options, jCell;
            if (!p.cellSelectable) return;
            if (g.editor && g.editor.editing) return;
            ...
            var firstColumn = (function ()
            {
                ...
                {
                    if (!g.columns[i].issystem && !g.columns[i].frozen) return g.columns[i];
                }
                return null;
            })();
            var lastColumn = (function ()
            {
                for (var i = g.columns.length-1; i >= 0; i--)
                {
                    if (!g.columns[i].issystem && !g.columns[i].frozen) return g.columns[i];
                }
                return null;
            ...
            if (!g.selectedCell)
            {
                g.selectedCell = g.getCellObj(g.rows[0], firstColumn);
                $(g.selectedCell).addClass("l-cellselected");
                return true;
            }
            ...
            if (type == "top")
            {
                var nextRow = $(g.selectedCell).parents("tr.l-grid-row").prev();
                if (!nextRow.length) return false;
                jCell = g.getCellObj(nextRow.get(0), g.getColumn( g.selectedCell));
                if (jCell) jCell = $(jCell);
            }
            else if (type == "down")
            {
                var nextRow = $(g.selectedCell).parents("tr.l-grid-row").next();
                if (!nextRow.length) return false;
                jCell = g.getCellObj(nextRow.get(0), g.getColumn(g.selectedCell));
                if (jCell) jCell = $(jCell);
            }
            else if (type == "left")
            {
                ...
                if (!jCell.length) //向上找
                {
                    ...
                    if (nextRow.length)
                    {
                        jCell = g.getCellObj(nextRow.get(0), lastColumn);
                        if (jCell) jCell = $(jCell);
                    }
                }
            }
            else
            {
                ...
                if (!jCell.length) //向下找
                {
                    ...
                    if (nextRow.length)
                    {
                        jCell = g.getCellObj(nextRow.get(0), firstColumn);
                        if (jCell) jCell = $(jCell);
                    }
                }
            }
            if (jCell.length)
            {
                g.selectCell(jCell.get(0)); 
                return true;
            }
            return false;

        },
        cover: function (content)
        {
            var g = this, p = this.options;
            ...
            var cell = g.selectedCell; 
            if (!cell) cell = g.getCellObj(g.rows[0], firstColumn);
            if (!cell) return;
            var column = g.getColumn(cell);
            var columnIndex = $.inArray(column, g.columns);
            var row = g.getRow($(cell).parents(".l-grid-row:first").get(0));
            var rowIndex = $.inArray(row, g.rows);
            var cRows = content.split("\n");
            var changedData = false;
            for (var i = 0; i < cRows.length - 1; i++)
            {
                (function (i)
                {
                    
                    var targetRow = g.rows[rowIndex + i];
                    var cCells = cRows[i].split("\t");
                    for (var j = 0; j < cCells.length; j++)
                    {
                        var targetColumn = g.columns[columnIndex + j]; 
                        if (!targetColumn || !targetColumn.name) continue;
                        if (targetRow[targetColumn.name] != cCells[j])
                        {
                            targetRow[targetColumn.name] = cCells[j];
                            changedData = true;
                        }
                    }
                })(i);
        ...
        selectCell : function(cell){
            var g = this, p = this.options;
            if (g.selectedCell) $(g.selectedCell).removeClass("l-cellselected");
            g.selectedCell = cell;
            $(g.selectedCell).addClass("l-cellselected");
        },

        _endEditTo: function (jcell,oldColumnIndex)
        {
            var g = this, p = this.options;

            if (jcell && jcell.length)
            {
                g.endEdit();
                g._applyEditor(jcell.get(0));
                var editcolumn = g.getColumn(jcell.get(0));
                var editcolumnIndex = $.inArray(editcolumn, g.columns);
                var diffWidth = 0;

                var start = Math.min(editcolumnIndex, oldColumnIndex);
                var end = Math.max(editcolumnIndex, oldColumnIndex);

                if (editcolumnIndex > oldColumnIndex)
                {
                    for (var i = start +1  ; i <= end ; i++)
                    {
                        diffWidth += g.columns[i]._width || g.columns[i].width;
                    }
                }
                else
                {
                ...
                if (g._updateEditorInScroll() === false)
                {
                    if (editcolumnIndex > oldColumnIndex)
                    {
                        g.gridbody.scrollLeft(scrollLeft + diffWidth);
                    } else
                    {
                        g.gridbody.scrollLeft(scrollLeft - diffWidth);
                    }
        ...
        //进入编辑状态
        startEdit: function (row,name)
        {
            var g = this, p = this.options;
            var jrow = g.getRowObj(row);
            if (!jrow) return;
            var jcell = $(jrow).find('td:first');
            //获取到下一个可编辑的列
            while ( !g._enabledEditByCell( jcell.get( 0 ), name ) )
            {
                jcell = jcell.next();
                if (!jcell.length) return;
            }
            if ( g._enabledEditByCell( jcell.get( 0 ), name ) )
            {
                setTimeout(function ()
                {
                    g._applyEditor(jcell.get(0));
        ...
        endEditToNext : function()
        {
            ...
            g._endEditTo(jnext, columnIndex);
        },


        endEditToPrev: function ()
        {
            var g = this, p = this.options;
            var editor = g.editor, jnext = null, jprev = null;
            if (editor)
            {
                ...
                jnext = $(cell);
                jprev = jnext.prev();
                if (!jprev.length) jprev = getPrevRowCell();//已经是当前行第一个单元格了
                if (jprev.length)
                {
                    //获取到下一个可编辑的列
                    while (!g._enabledEditByCell(jprev.get(0)))
                    {
                        jnext = jprev;
                        jprev = jprev.prev();
                        if (!jprev.length)//已经是当前行第一个单元格了
                        {
                            jprev = getPrevRowCell();
                            if (!jprev.length) break;
                        }
                    }
                }
                //获取上一行最后一个列对象
                function getPrevRowCell()
                {
                    return jnext.parent("tr").prev(".l-grid-row").find("td:last");
                }
            }

            g._endEditTo(jprev, columnIndex);
        ...
        endEditToUp: function ()
        {
            var g = this, p = this.options;
            var editor = g.editor, jtargetCell = null;
            if (editor)
            {
                var editParm = editor.editParm;
                var column = editParm.column;
                var columnIndex = $.inArray(column, g.columns);
                var cell = g.getCellObj(editParm.record, editParm.column); 
                var cellIndex = $(cell).parent("tr").find("td").index(cell);

                jtargetCell = $(cell).parent("tr").prev(".l-grid-row").find("td:eq(" + cellIndex + ")");
                 
            } 
            if (jtargetCell && jtargetCell.length)
            {
                g.endEdit();
                g._applyEditor(jtargetCell.get(0));
                var scrollLeft = g.gridbody.scrollLeft();
                var scrollTop = g.gridbody.scrollTop();
                if (g._updateEditorInScroll() === false)
                {
                    g.gridbody.scrollTop(scrollTop - p.rowHeight -20);
                } 
            }
        },
        endEditToDown: function ()
        {
            var g = this, p = this.options;
            var editor = g.editor, jtargetCell = null;
            if (editor)
            {
                ...
                jtargetCell = $(cell).parent("tr").next(".l-grid-row").find("td:eq(" + cellIndex + ")");
        ...
        {
            return this.options.groupColumnName || this.options.groupValueGetter ? true : false;
        },
        ...
        */
        updateCell: function (arg, value, rowParm)
        {
            ...
            if (column.name)
            {
                ...
                if (rowdata[p.statusName] != 'add')
                {
                    rowdata[p.statusName] = g._isChanged(rowdata) ? 'update' : 'nochanged';
                    if (rowdata[p.statusName] == "update")
                    {
                        g.isDataChanged = true;
                    }
                }
        ...
        {
            var g = this, p = this.options;
            if (!g.rows) return false;

            for (var i = 0; i < g.rows.length; i++)
            {
                if (g._isChanged(g.rows[i])) return true;
            }
            return false;
        },
        ...
        _isValueEqual: function (cellvalue1, cellvalue2)
        {
            var g = this, p = this.options;
            if (!cellvalue1 && !cellvalue2) return true;
            if (!cellvalue1 && cellvalue2) return false;
            if (cellvalue1 && !cellvalue2) return false;
            if ($.isArray(cellvalue1) && $.isArray(cellvalue2) && cellvalue1.length && cellvalue2.length)
            {
                return g._isValueEqual(cellvalue1[0], cellvalue2[0]);
            }
            if (!cellvalue1.toString || !cellvalue2.toString())
            {
                return cellvalue1 == cellvalue2;
            }
            var s1 = cellvalue1.toString();
            var s2 = cellvalue2.toString();
            if (s1 == s2) return true;

        },

        _isChanged : function(row){
             
            var g = this, p = this.options;
            var rawRow = g._getRawRow(row);
            if (!rawRow) return true;
            for (var name in rawRow)
            {
                if (name.indexOf('_') == 0) continue;
                if (!g._isValueEqual(rawRow[name] , row[name])) return true;
            }
            for (var name in row)
            {
                if (name.indexOf('_') == 0) continue;
                if (!(name in rawRow)) return true;
            }
            return false;
        },
        ...
        reRender: function (e)
        {
            ...
            if (p.disabledReRender) return; //可以通过开关暂时禁用，常用于 多次addRow 时可以先禁用 等最后一次addRow 再开启
            ...
            function reRenderTotal(column)
            {
                if (p.totalRender)
                {
                    ...
                    $(".l-panel-bar", g.element).before('<div class="l-panel-bar-total">' + p.totalRender.call(g, g.data, g.filteredData) + '</div>');
                }
                if (p.totalRender2)
                {
                    g.gridbody.find(">div.gridtotal").remvoe();
                    g.gridbody.append('<div class="gridtotal">' + p.totalRender2(g.data, g.filteredData) + '</div>');
                }
        ...
        {
            var g = this, grid = this, p = this.options;

            var rows = [];
            var rawRows = g.rawRows || [];
            var rows = [];
            for (var i = 0; i < rawRows.length; i++)
            {
                var rawRow = $.extend(true, {}, rawRows[i]);
                delete rawRow.__row;
                rows.push(rawRow);

                rawRows[i].__row = rawRow; //重定向
            }
            g.currentData[p.root] = rows;
            g.deletedRows = [];
            g.selected = [];
            g.reRender();
        },
        ...
        { 
            ...
            var hcell = $(".l-grid-hd-row .l-grid-hd-cell[columnname=" + columnName + "]", g.element);
            var sort = $(".l-grid-hd-cell-sort:first", hcell);
            var hcelltext = $(".l-grid-hd-cell-text", hcell);
              
            if (p.sortName == columnName && sort.hasClass("l-grid-hd-cell-sort-" + sortOrder))
            { 
                return;
            }
            hcell.removeClass("l-grid-hd-cell-desc l-grid-hd-cell-asc").addClass("l-grid-hd-cell-" + sortOrder);
            hcelltext.find(".l-grid-hd-cell-sort").remove();

            hcelltext.after("<span class='l-grid-hd-cell-sort l-grid-hd-cell-sort-" + sortOrder + "'>&nbsp;&nbsp;</span>");
        ...
        changePage: function (ctype)
        {
            ...
            var g = this, p = this.options;
            if (g.loading) return true;
            if (p.dataAction != "local" && g.dataIsChanged() && !confirm(p.isContinueByDataChanged))
                return false;
        ...
        toggleCol: function (columnparm, visible, toggleByPopup)
        {
            ...
            {
                g.trigger('toggleColumn', [column, visible]);
            }

            g.trigger('ColumnChange', [{
        ...
        { 
            ...
            g.reRender();
            g.trigger('ColumnChange', [{
                from: from, to: to, isAfter: isAfter
            }]);
        },
        ...
        _initColumns: function ()
        {
            ...
            //201612增加align的初始化:文本类型居左、数值类型居右、日期类型居中显示
            function setColumn(column, level, pid, previd)
            {
                ...
                if (column.type == "text" && !column.align)
                {
                    column.align = "left";
                }
                if (column.type == "date" && !column.align)
                {
                    column.align = "center";
                }
                if ((column.type == "int" || column.type == "number" || column.type == "currency" || column.type == "percent") && !column.align)
                {
                    column.align = "right";
                } 
        ...
        _createHeaderCell: function (column)
        {
            ...
            if (column.display && column.display != "")
                ...
                headerText = "&nbsp;";
            if (p.isShowFilter && column.type && column.name)
            {
                $("div:first", jcell).append('<div class="header-filterbtn" data-name="' + column.name + '"></div>');
            }
            ...
            if (p.usePager)
            {
                ...
                if (p.switchPageSizeApplyComboBox && $.fn.ligerComboBox)
                {
                    $(".l-bar-selectpagesize select", g.toolbar).ligerComboBox(
                    {
                        ...
                        {
                            if (p.url && g.dataIsChanged() && !confirm(p.isContinueByDataChanged))
                                return false;
                            return true;
                        },
                        width: 45
        ...
        _fillGridBody: function (data, frozen,sourceType)
        {
            ...
            if (g.enabledGroup()) //启用分组模式
            {
                ...
                for (var rowparm in data)
                {
                    ...
                    var groupValue = p.groupValueGetter ? p.groupValueGetter.call(g, item) : item[p.groupColumnName];
                    var valueIndex = $.inArray(groupValue, groups);
                    if (valueIndex == -1)
                    {
            ...
            //分组时不需要            
            if (!g.enabledGroup() || p.totalAllData)
            {
                //创建汇总行
        ...
        _anyRows: function (data)
        {
        	var g = this, p = this.options;
        	data = data || g.rows;
        	for (var i = 0; i < data.length; i++)
        	{
        		if (data[i] && data[i][p.statusName] != "delete")
        		{
        			return true;
        		}
        	}
        ...
        _setRowStyles: function (value)
        {
            var g = this, p = this.options;

            if (g.jstyle) g.jstyle.remove();
            if (!value || !value.length) return;

            g.jstyle = $("<style type='text/css'></style>").appendTo($("head"));
            var styleContents = [];

            for (var ii = 0; ii < p.rowStyles.length; ii++) {
                var rowStyle = p.rowStyles[ii];
                styleContents.push('.l-gridrow-user-' + ii + ' .l-grid-row-cell{ ' + rowStyle.value + ' }');
            }
            
            if (p.mouseoverRowBgColor) {
                styleContents.push('.l-grid-row-over .l-grid-row-cell {background: ' + p.mouseoverRowBgColor + '; }');
            }
            if (p.selectedRowBgColor) {
                styleContents.push('.l-selected .l-grid-row-cell {background: ' + p.selectedRowBgColor + '; }');
            }
            g.jstyle.text(styleContents.join(' '));
        },
        _getHtmlFromData: function (data, frozen)
        {
            ...
            for (var i = 0, l = data.length; i < l; i++)
            {
                ...
                if (!frozen && g.enabledCheckbox() && p.isChecked && p.isChecked.call(g, item))
                {
                    g.select(item, false, true);
                ...
                if (p.rowStyles)
                {
                    ...
                    {
                        var rowStyle = p.rowStyles[ii];
                        var rowCheck = $.isFunction(rowStyle.rule) ? rowStyle.rule.call(g, item, i) : (function () {
                            try {
                                var fn = null;
                                eval("fn = function(row,index){ " + (p.expressionInit || "") + " return " + rowStyle.rule + "}");
                                return fn(item, i);
                            }
                            catch (e) {
                                return "";
                        ...
                        if (rowCheck === true)
                        {
                            gridhtmlarr.push(' l-gridrow-user-' + ii);
                            break;
                        }
                    }
                }
                ...
                $(g.columns).each(function (columnindex, column)
                {
                    ...
                    if (this.isrownumber)
                    {
                        ...
                        var rownumber = parseInt(item['__index']) + 1;
                        if (p.rownumbersByPage && p.usePager)
                        {
                            rownumber =(p.newPage - 1) * p.pageSize + rownumber;
                        } 
                        gridhtmlarr.push(rownumber);
                    ...
                    if (column.style)
                    {
                        ...
                        {
                            gridhtmlarr.push(column.style);
                        }
                        else
                        {
                            var vs = [];
                            for (var name in column.style)
                            {
                                var proName = name.replace(/[A-Z]/g, function (a) { return "-" + a.toLowerCase() });
                                var proValue = $.isFunction(column.style[name]) ? column.style[name].call(g,item,i) : (function () {
                                    try {
                                        var fn = null;
                                        eval("fn = function(row,index){ " + (p.expressionInit || "") + " return " + column.style[name] + "}");
                                        return fn(item, i);
                                    }
                                    catch (e) {
                                        return "";
                                ...
                                vs.push(proName + ":" + proValue + ";");
                            }
                            gridhtmlarr.push(vs.join(''));
                        }
                    }
                    if (column._hide)
                    {
        ...
        _getCellHtml: function (rowdata, column)
        {
            ...
            { 
                ...
                if (p.fixedCellHeight) htmlarr.push(' style="height:' + p.rowHeight + 'px;"');
                htmlarr.push('>');
                var index = parseInt(rowdata['__index']) + 1;
                 
                if (p.rownumbersByPage && p.usePager)
                {
                    index = (p.newPage-1) * p.pageSize  + index;
                }
                htmlarr.push(index);
                htmlarr.push('</div>');
                return htmlarr.join('');
        ...
        _getCellContent: function (rowdata, column)
        {
            ...
            if (column.isrownumber)   
            {
                var rownumber = parseInt(rowdata['__index']) + 1;
                if (p.rownumbersByPage && p.usePager)
                {
                    return (p.newPage - 1) * p.pageSize + rownumber;
                }
                else
                {
                    return rownumber;
            ...
            if (column.render)
            {
                if (typeof (column.render) === "string")
                {
                    content = (function ()
                    {
                        ...
                        {
                            var fn = null;
                            eval("fn = " + column.render);
                            return fn.call(g, rowdata, rowindex, value, column);
                        }
                        catch (e)
                        {
                            return "";
            ...
            else if (column.exp)
            {
                ...
                content = (function ()
                {
                    ...
                    {
                        var fn = null;
                        eval("fn = function(row,data,rows,index){ " + (p.expressionInit || "") + " return " + column.exp + "}");
                        return fn(rowdata,rowdata, g.rows, rowindex);
                    }
                    catch (e)
                    {
                        return "";
                ...
                if (column.type && p.formatters[column.type] && content)
                {
                    content = content.toString();
                    content = p.formatters[column.type].call(g, content, column);
                }
            }
        ...
        _applyEditor: function (obj)
        {
            ...
            if ($.isFunction(columnEditor))
            {
                columnEditor = columnEditor.call(g, rowdata);
                g.currentColumnEditor = columnEditor;
            }
            if (columnEditor.type && p.editors[columnEditor.type])
            {
                var currentdata = g._getValueByName(rowdata, columnname);
                var editParm = { grid : g, record: rowdata, old : $.extend(true,{} , rowdata), value: currentdata, column: column, rowindex: rowindex };
                ...
                var editor = p.editors[columnEditor.type],
                ...
                container.attr({
                    _left: left,
                    _top: top,
                    _scrollleft: g.gridbody.scrollLeft(),
                    _scrolltop: g.gridbody.scrollTop()
                });
                g.editorContainer = container;
                g.editorCell = rowcell;
                if (column.textField) editParm.text = g._getValueByName(rowdata, column.textField);
                var editorInput = g._createEditor(editor, container, editParm, width, height - 1);
                if ($.isFunction(column.editor))
                {
                    g.currentColumnEditor = columnEditor;
                }
                ...
                { 
                    var newValue = editor.getValue.call(g,editorInput, editParm);
                    if (column.textField && editor.getText)
                    {
                        editParm.text = editor.getText.call(g, editorInput, editParm);
                    }
                    if (editor.getSelected)
                    {
                        editParm.selected = editor.getSelected.call(g, editorInput, editParm);
                    }
                    ...
                    g._checkEditAndUpdateCell(editParm, editorInput);
                    
                });
            }
        },
        _checkEditAndUpdateCell: function (editParm, editorInput)
        {
            var g = this, p = this.options;

             
            var evenResult = g.trigger('beforeSubmitEdit', [editParm]);
            if (evenResult == false)
                return;
            if ($.isFunction(evenResult))
            {
        ...
        _getTotalCellContent: function (column, data, isAllData)
        {
            var g = this, p = this.options;
            var totalsummaryArr = [];
            var totalSummary = isAllData ? (column.totalSummaryAll || column.totalSummary) : column.totalSummary;
            if (totalSummary)
            {
                ...
                if (totalSummary.render)
                {
                    var renderhtml = totalSummary.render(info, column, g.data);
                    totalsummaryArr.push(renderhtml);
                }
                else if (totalSummary.type && info)
                {
                    var types = totalSummary.type.split(',');
                    if (isExist('sum'))
        ...
        _getTotalSummaryHtml: function (data, classCssName, frozen,isAllData)
        {
            var g = this, p = this.options;
            var totalsummaryArr = [];
            if (classCssName)
            ...
            $(g.columns).each(function (columnindex, column)
            {
                ...
                if (column.totalSummary && column.totalSummary.align)
                    totalsummaryArr.push(' style="text-Align:' + column.totalSummary.align + ';"');
                if (column.align)
                    totalsummaryArr.push(' style="text-Align:' + column.align + ';"');
                totalsummaryArr.push('>');
                totalsummaryArr.push(g._getTotalCellContent(column, data, isAllData));
        ...
        _bulidTotalSummary: function (frozen)
        {
            ...
            var totalRow = $(g._getTotalSummaryHtml(g.currentData[p.root], null, frozen,true));
        ...
        _isEditing: function (jobjs)
        {
            ...
            if (jobjs.hasClass("ui-datepicker") || jobjs.hasClass("ui-datepicker-header")) return true;
            //判断是否位于编辑器弹出的框
        ...
        _getSrcElementByEvent: function (e)
        {
            ...
            var r = {
                ...
                header_filterbtn: fn("header-filterbtn"),
            ...
            if (r.gridheader)
            {
                ...
                r.filterbtn = fn("header-filterbtn");
        ...
        _updateEditorInScroll: function ()
        {
            var g = this, p = this.options;
            var scrollLeft = g.gridbody.scrollLeft();
            var scrollTop = g.gridbody.scrollTop();
            ...
            if (g.isEditing() && g.editorContainer)
            {
                g.editorContainer.css("top", parseInt(g.editorContainer.attr("_top")) - (scrollTop - parseInt(g.editorContainer.attr("_scrolltop"))));
                g.editorContainer.css("left", parseInt(g.editorContainer.attr("_left")) - (scrollLeft - parseInt(g.editorContainer.attr("_scrollleft"))));
                 
                if (!p.usePager)
                {
                    g.toolbar.css("height", 1);
                    g.toolbar.show();
                }
                if ($(g.editorCell).offset().top < g.gridheader.offset().top + g.gridheader.height())
                {
                    g.editorContainer.hide();
                    return false;
                }
                else if ($(g.editorCell).offset().top > g.toolbar.offset().top - $(g.editorCell).height())
                {
                    g.editorContainer.hide();
                    return false;
                }
                else if ($(g.editorCell).offset().left < g.gridbody.offset().left)
                {
                    g.editorContainer.hide();
                    return false;
                }
                else if ($(g.editorCell).offset().left + g.editorContainer.width() > g.gridbody.offset().left + g.gridbody.width())
                {
                    g.editorContainer.hide();
                    return false;
                }
        ...
        {
            var g = this, p = this.options;
            g.grid.bind("selectstart.grid", function ()
            {
                //return false;
            });
            ...
            {
                if (g.dataIsChanged() && p.dataAction != "local" && !confirm(p.isContinueByDataChanged))
                    return false;
                p.newPage = 1;
                p.pageSize = this.value;
                $('span.pcontrol :text', g.toolbar).val("1");
                if (p.dataAction != "local")
                {
                    g.loadData(p.where);
                }
            ...
            if ($.fn.ligerDrag && p.colDraggable)
            {
                ...
                g.gridheader.add(g.f.gridheader).ligerDrag({
                    ...
                    onStartDrag: function (current, e)
                    {
                        ...
                        this.set('cursor', 'default');
                        var src = g._getSrcElementByEvent(e);
                        if (!src.hcell || !src.column || src.column.issystem || src.filterbtn
                            //|| src.hcelltext
                            )
                        {
                            return false;
        ...
        _onClick: function (e)
        {
            ...
            if ($(obj).is("a.hyperlink")) return;
             
            var src = g._getSrcElementByEvent(e);
             
            if (src.header_filterbtn)
            {
                g._toggleFilter(src.header_filterbtn);
                return;
            }
            if (src.out)
            {
            ...
            else if (src.hcelltext) //排序
            {
                ...
                if (p.url && p.dataAction != "local" && g.dataIsChanged() && !confirm(p.isContinueByDataChanged)) return;
                var sort = $(".l-grid-hd-cell-sort:first", hcell);
                var columnName = src.column.name;
                if (!columnName) return;

                var sortType = sort.hasClass("l-grid-hd-cell-sort-asc") ? "desc" : "asc";
                g.trigger('ChangeSort', [columnName, sortType]);
                g.changeSort(columnName, sortType);
            ...
            else if (src.row && g.enabledCheckbox()) //复选框选择行
            {
                ...
                if (p.enabledEdit && !p.selectableInEdit) selectRowButtonOnly = true;
                ...
                if ((src.checkbox || !selectRowButtonOnly) && p.selectable != false)
                {
                    ...
                    if (met == "select" && !src.checkbox && !selectRowButtonOnly && p.enabledEdit)
                    {
                        g.select(src.data, true);
                    } else
                    { 
                        g[met](src.data);
                    }
                ...
                if (!src.checkbox && src.cell)
                {
                    if (p.cellSelectable)
                    {
                        if ($(src.cell).hasClass("l-cellselected"))
                        {
                        ...
                        {
                            if (g.selectedCell) $(g.selectedCell).removeClass("l-cellselected");
                            g.selectedCell = src.cell;
                            $(src.cell).addClass("l-cellselected");
                        }
                    }
                    else if (p.enabledEdit && p.clickToEdit)
                    {
                        g._applyEditor(src.cell);
                    }
                }
        ...
        select: function (rowParm,isSingle,noEvent)
        {
            var g = this, p = this.options;
            var rowdata = g.getRow(rowParm);
            var rowid = rowdata['__id'];
            ...
            if (g.shiftKey && p.enabledShiftKey)
            {
                var startRow = g.selected && g.selected.length ? g.selected[0] : g.rows[0];
                if (startRow == rowdata) return;
                var startIndex = $.inArray(startRow, g.rows);
                var endIndex = $.inArray(rowdata, g.rows);
                ...
                {
                    var row = g.rows[i];
                    var cur_rowobj = g.getRowObj(row);
                    var cur_rowobj1 = g.getRowObj(row, true); 
                    $(cur_rowobj).addClass("l-selected");
                    $(cur_rowobj1).addClass("l-selected");
                    g.selected[g.selected.length] = row; 
                }

            }
            else if (isSingle || (!g.enabledCheckbox() && !g.ctrlKey) || p.isSingleCheck) //单选
            {
        ...
        _onResize: function ()
        {
            ...
            {
                var windowHeight = g._getContainerHeight();
                ...
                if (typeof (p.height) == "string" && p.height.indexOf('%') > 0)
                {
                    ...
                    {
                        var cot = g._getContainerOffsetTop(); 
                        h -= (g.grid.offset().top - ( cot ? cot : parseInt( $('body').css('paddingTop'))));
                    } 
                }
                else
                {
        ...
        filterGrid: function ()
        {
            ...
            for (var i = 0; i < g.columnFilters.length; i++)
            {
                var o = g.columnFilters[i];
                var content = o.content;

                if (content.filter && content.filter.rules && content.filter.rules.length) //自定义过滤
                {
                    filterData.groups.push(content.filter);
                }
                else if (content.values && content.values.length) //选择了某些值
                {
                    var valueGroup = {
                        rules: [],
                        op: 'or'
                    };
                    $(content.values).each(function (i,value)
                    { 
                        valueGroup.rules.push({
                            field: o.name,
                            value: value,
                            op: 'equal'
                        });
                    ...
                    filterData.groups.push(valueGroup);
                }
            }
              
            var fn = $.ligerFilter.getFilterFunction(filterData); //转换为过滤函数
            g.currentFilterFn = fn;
            g.loadData(fn);//过滤grid

        },
        clearColumnFilter: function (columnName)
        { 
            var g = this, p = this.options;
            if (!g.columnFilters) g.columnFilters = [];
            for (var i = 0; i < g.columnFilters.length; i++)
            {
                if (g.columnFilters[i].name == columnName)
                {
                    g.columnFilters.splice(i, 1);
                    break;
                }
            } 
            $(".l-grid-hd-row .l-grid-hd-cell[columnname="+columnName+"]", g.element).removeClass("l-header-hasfilter");
            g.filterGrid();
        },
        clearAllColumnFilter: function ()
        {
            var g = this, p = this.options;
            if (!g.columnFilters || !g.columnFilters.length) return;
            g.columnFilters = [];
            $(".l-grid-hd-row .l-grid-hd-cell", g.element).removeClass("l-header-hasfilter");
            g.filterGrid();
        },
        findColumnFilter: function (columnName)
        {
            var g = this, p = this.options;
            for (var i = 0; g.columnFilters && i < g.columnFilters.length; i++)
            {
                if (g.columnFilters[i].name == columnName) return g.columnFilters[i];
            }
            return null;
        },
        setColumnFilter: function (columnName, filter)
        {
            var g = this, p = this.options;
            g.columnFilters = g.columnFilters || [];
            var columnFilter = g.findColumnFilter(columnName);
            if (!columnFilter)
            {
                /*
                {
                ...
                */
                g.columnFilters.push({
                    name: columnName,
                    content: filter
                });
            } else
            {
                columnFilter.content = filter;
            }
            $(".l-grid-hd-row .l-grid-hd-cell[columnname=" + columnName + "]", g.element).addClass("l-header-hasfilter");
            g.filterGrid(); 
        },
        ...
        {
            var g = this, p = this.options;
            var jtarget_listbox = $('<div></div>');

            jtarget_listbox.css("padding", 10).css("marginLeft", 35);
            jtarget_listbox.append('<div style="margin-bottom:8px"><input type="text" class="searchtext l-text" style="width:160px" /></div>');

            jtarget_listbox.append('<div class="listbox"></div>');

            var jtarget_btns = $('<div class="filterbox-bottom"></div>');
            jtarget_btns.css("padding", 10).css("marginLeft", 35);

            var jcanel = $('<input type="button" class="btn" value="取消" />').appendTo(jtarget_btns);
            var jok = $('<input type="button" class="btn" value="确定" />').appendTo(jtarget_btns);
            jcanel.click(function ()
            {
                menu.hide();
            });
            jok.click(function ()
            {
                var value = menu.listbox.getValue();
                 
                if (menu.listbox.isAllSelected())
                {
                    g.clearColumnFilter(menu.currentColumn.name);
                }
                else if (value)
                {
                    g.setColumnFilter(menu.currentColumn.name, {
                        values: value.split(";")
                    });
                }
                menu.hide();
            ...
            var width = 220;
            jtarget_listbox.find(".searchtext").width(width).bind('keyup', function ()
            { 
                var key = this.value;
                menu.listbox.selectAll(key);
            });
            ...
            var operators = {};
                ...
                var menuItems = [
                        {
                            text: '升序', id: "asc", icon: 'asc', click: menuClick
                        },
                        {
                            text: '倒序', id: "desc", icon: 'desc', click: menuClick
                        },
                         {
                             text: '清除过滤条件', id: 'clear', icon: 'clear', click: menuClick
                         },
                        {
                            line: true
                        }
                ...
                var item_text = {
                    text: '文本过滤', id: 'btns_text', children: []
                }; 
                $(operators.text).each(function ()
                {
                    item_text.children.push({
                        text: texts[this], id: 'btn_' + this, click: menuClick
                    });
                });
                item_text.children.push({ line: true });
                item_text.children.push({ text: '自定义过滤', id: 'btn_custom' });

                var item_date = {
                    text: '日期过滤', id: 'btns_date', children: []
                };

                $(operators.date).each(function ()
                {
                    item_date.children.push({
                        text: texts[this], id: 'btn_' + this, click: menuClick
                    });
                });
                item_date.children.push({ line: true });
                item_date.children.push({ text: '自定义过滤', id: 'btn_custom' });


                var item_number = {
                    text: '数值过滤', id: 'btns_number', children: []
                };

                $(operators.number).each(function ()
                {
                    item_number.children.push({
                        text: texts[this], id: 'btn_' + this, click: menuClick
                    });
                });
                item_number.children.push({ line: true });
                item_number.children.push({ text: '自定义过滤', id: 'btn_custom' });


                menuItems.push(item_text);
                menuItems.push(item_date);
                menuItems.push(item_number);

                menuItems.push({ line: true });
                menuItems.push({
                    id: 'values',
                    target: jtarget_listbox
                });
                menuItems.push({ line: true });
                menuItems.push({ target: jtarget_btns });




            var menu = g.filterMenu = $.ligerMenu({
                top: 100, left: 100,
                width: 270,
                subMenuWidth: 120, items:menuItems
                    
            });

            menu.listbox =  jtarget_listbox.find(".listbox").ligerListBox(
            {
                isShowCheckBox: true,
                isMultiSelect: true,
                ...
                onSelected: function (v)
                {
                    if (!this.getValue())
                    {
                        jok.attr("disabled", "disabled");
                    } else
                    {
                        jok.removeAttr("disabled");
                    }
                }
            });
            menu.listbox.selectAll();
             

            function menuClick(item)
            {
                var currentType = menu.currentColumn.type || "text";
                var currentFilter = g.findColumnFilter(menu.currentColumn.name);
                ...
                if (item.id == "desc")
                {
                    g.changeSort(menu.currentColumn.name, 'desc');
                    return;
                }
                if (item.id == "asc")
                {
                    g.changeSort(menu.currentColumn.name, 'asc');
                    return;
                }
                if (item.id.indexOf('btn_') != -1)
                {
                    var filtercontainer = $('<div></div>').width(380).height(120).hide();
                    ...
                    $.ligerDialog.open({
                        ...
                        buttons: [
                            {
                                text: '确定', cls: 'l-dialog-btn-highlight', onclick: function (item, dialog)
                                {
                                    ...
                                    if (data.op1 || data.op2)
                                    {
                                        ...
                                        if (data.op1)
                                        {
                                            ...
                                            if (value == g.NULLITEM.text) value = g.NULLITEM.id;

                                            filter.rules.push({
                                                field: menu.currentColumn.name,
                                                op: data.op1,
                                                type : currentType,
                                                value: value
                                            });
                    ...
                    var operatorsItems = [{ text: '&nbsp', id: '' }];
                     
                    $(operators[currentType]).each(function (i, type)
                    {
                        operatorsItems.push({
                    ...
                    var currentOperator = item.id.replace("btn_", "");
                    
                    var filterForm = filtercontainer.ligerForm({
                        prefixID: new Date().getTime(),
                        fields: [
                            {
                                hideLabel: true,
                                type: 'select',
                    ...
                    if (currentFilter != null && currentFilter.content && currentFilter.content.formData && currentFilter.content.formData.op1 == currentOperator)
                    {
                        filterForm.setData(currentFilter.content.formData);
                    } else
                    {
                        filterForm.setData({
                            op1: currentOperator
                    ...
                    filtercontainer.find(".l-radiolist").attr("style", "");
                }
            }
        },
        _toggleFilter: function (obj)
        { 
            var g = this, p = this.options;
            ...
            var menu = g.filterMenu;
            if (menu.menu.is(":visible"))
            {
                menu.hide();
            } else
            {
                var t = $(obj).offset().top + 16, l = $(obj).offset().left + 16 - 270;
                if (l < 0) l = 0;

                menu.show({ top: t, left: l });

                var groups = [], data = []; //分组列名数组  
                g.NULLITEM = {
                    id: 'NULL',
                    text: '(空)'
                };
                var hasNullitem = false; 
                for (var i in g.rawRows)
                {
                    var row = g.rawRows[i];
                    ...
                    var valueIndex = $.inArray(value, groups);
                    if (valueIndex == -1)
                    {
                        groups.push(value);
                        data.push({
                ...
                if (hasNullitem)
                {
                    data.splice(0, 0, g.NULLITEM);
                }
                menu.setMenuText('clear', '从"' + column.display + '"中清除过滤条件');
                menu.listbox.setData(data);
                var columnFilter = g.findColumnFilter(column.name);
                 
                if (columnFilter)
                {
                    var o = columnFilter.content;

                    if (o.values)
                    {
                        menu.listbox.setValue(o.values.join(';'));
                    }
                    else
                    {
                        menu.listbox.selectAll();
                    ...
                    menu.setEnabled('clear');
                } else
                {

                    menu.listbox.selectAll();

                    menu.setDisabled('clear');
                }

                var menuIds = ["btns_date", "btns_text",'btns_number'];


                menu.setVisible(menuIds, false);

                var defaultId = "btns_text";
                if ($.inArray("btns_" + column.type, menuIds) != -1) defaultId = "btns_" + column.type;

                menu.setVisible(defaultId, true);

                menu.clearIcon(defaultId);
                 
                if (columnFilter)
                {
                    var o = columnFilter.content;

                    if (o.filter && o.formData)
                    {
                        menu.setIcon(defaultId, 'selected');

                        if (o.formData.op2 && o.formData.value2)
                        {
                            menu.setIcon('btn_custom', 'selected', defaultId);
                        } else if (o.formData.op1)
                        {
                            menu.setIcon('btn_' + o.formData.op1, 'selected',defaultId);
                        }

                    }  
                }

                menu.currentColumn = column;
                menu.filterSource = data;
