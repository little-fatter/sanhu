...
(function ($)
{
    ...
    $.ligerui.controls.Panel.ligerExtend($.ligerui.core.UIComponent, {
        ...
        refresh : function()
        {
            ...
            else if (g.jtarget && g.jtarget.length)
            {
                var target = g.jtarget.get(0);
                if ($.isFunction(target.panelReload))
                {
                    target.panelReload.call(g, {
                        target: target 
                    });
                }
            }
        ...
        _setTarget: function (jtarget)
        {
            var g = this, p = this.options;
            var content = g.panel.find(".l-panel-content:first");
            if (jtarget)
            { 
                jtarget.appendTo(content);
                g.jtarget = jtarget;
                g.trigger('loaded');
            }
        },
