...
(function ($)
{
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        _render: function ()
        {
            ...
            if ($.isFunction(p.beforeRender))
            {
            	if (p.beforeRender.call(g) == false) return;
            }
            var tmpId = ""; 
            g.set(p, true);
        ...
        doMax: function (slide)
        {
            ...
            if (slide)
            {
                g.dialog.body.animate({ width: width  }, p.slide);
                g.dialog.animate({ left: left, top: top }, p.slide);
                g.dialog.content.animate({ height: height - g.dialog.buttons.outerHeight() }, p.slide, function ()
                {
                    g._onReisze();
                });
            }
