...
(function ($)
{
    ...
    $.ligerDefaults.Form.editors.textarea =
    {
        create: function (container, editParm, p)
        {
            ...
            var prefixID = $.isFunction(p.prefixID) ? p.prefixID(this) : (p.prefixID || "");
            var field = editParm.field;
            var id = (prefixID || "") + field.name;
            ...
            {
                if (field.editor && field.editor.onChangeValue)
                {
                    field.editor.onChangeValue(this.value);
                }
            });


            if (field.editor && field.editor.height)
            {
                editor.height(field.editor.height);
            }
    ...
    $.ligerui.controls.Form.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _initCheckboxEvent : function(){
            var g = this, p = this.options;
            $( "li.l-label-checkbox", g.element ).bind( "click", function ( e )
            {
                var editor = liger.get( $( this ).parent().find( ":checkbox" ).attr( "ligeruiid" ) );
                if ( editor && editor._checkboxClick )
                {
                    editor._checkboxClick.call( editor );
                }
                e.stopPropagation();
            } );
        },
            ...
            if (fields && fields.length)
            {
                ...
                $(groups).each(function (groupIndex, group)
                {
                    ...
                    { 
                        ...
                        if (!name && field.type != "html" && field.type != "render") return;
                        ...
                        if (field.style)
                        {
                            out.push(field.style + ';');
                        }
                        if (field.position)
                        {
                            out.push("position: absolute;");
                            if (field.position.left) out.push("left: " + field.position.left + ";");
                            if (field.position.top) out.push("top: " + field.position.top + ";");
                            if (field.position.right) out.push("right: " + field.position.right + ";");
                            if (field.position.bottom) out.push("bottom: " + field.position.bottom + ";");
                        }
	    ...
	    var jform = $(g.element);
            ...
            if (g._hasExp())
            {
                $(".l-text :text", g.element).bind('blur', function ()
                {
                    var formData = g.getData();
                     
                    var uiid = $(this).attr("ligeruiid");
                    var uiname = $(this).attr("formname");

                    setExpValueInFields(p.fields);
                    if (p.tab && p.tab.items)
                    {
                        for (var i = 0; i < p.tab.items.length; i++)
                        {
                            var item = p.tab.items[i];
                            setExpValueInFields(item.fields, i);
                        }
                    }

                    function setExpValueInFields(fields, tabIndex)
                    {
                        $(fields).each(function (fieldIndex, field)
                        {
                            var name = field.name,
                            ...
                            if (field.exp)
                            {
                                if (uiname && field.exp.indexOf(uiname) == -1) return;
                                if (editor.control && editor.control.id == uiid) return;
                                var value = (function ()
                                {
                                    try
                                    {
                                        var fn = null;
                                        eval("fn = function(data){ " + (p.expressionInit || "") + " return " + field.exp + "}");
                                        return fn(formData);
                                    }
                                    catch (e)
                                    {
        ...
        {
            var g = this, p = this.options;
            if (hasExpInFields(p.fields)) return true;
            if (p.tab && p.tab.items)
            {
                for (var i = 0; i < p.tab.items.length; i++)
                {
                    var item = p.tab.items[i]; 
                    if (hasExpInFields(item.fields,i)) return true;
                }
            }
            return false; 
            function hasExpInFields(fields, tabIndex)
            {
                for (var i = 0; fields && i < fields.length; i++)
                {
                    var field = fields[i];
                    if (field.exp) return true;
                } 
            }
        },
        getChanges: function ()
        ...
        { 
            var g = this, p = this.options;
            setLabelInFields(p.fields);
            if (p.tab && p.tab.items)
            {
                for (var i = 0; i < p.tab.items.length; i++)
                {
                    var item = p.tab.items[i];
                    setLabelInFields(item.fields, i);
                }
            }
            function setLabelInFields(fields, tabIndex)
            {
                $(fields).each(function (fieldIndex, field)
                {
                    var name = field.name;
                    if (name != fieldName) return;
                    g._setLabel(tabIndex, fieldIndex, label);
                });
            }
        },
        setFieldWidth: function (fieldName, width)
        {
            var g = this, p = this.options;
            eachInFields(p.fields);
            if (p.tab && p.tab.items)
            {
                for (var i = 0; i < p.tab.items.length; i++)
                {
                    var item = p.tab.items[i];
                    eachInFields(item.fields, i);
                }
            }
            function eachInFields(fields, tabIndex)
            {
                $(fields).each(function (fieldIndex, field)
                {
                    var name = field.name;
                    if (name != fieldName) return;
                    setwidth(tabIndex, fieldIndex, width);
                    g.getEditor(fieldName).set('width', width - 2);
                });
            }

            function setwidth(tabindex, fieldindex, width)
            {
                var g = this, p = this.options;
                if (tabindex == null)
                {
                    $("li.l-fieldcontainer[fieldindex=" + fieldindex + "] .fieldcontrol", g.element).width(width);
                }
                else
                {
                    $("div." + p.clsTabContent + "[data-index=" + tabindex + "] li.l-fieldcontainer[fieldindex=" + fieldindex + "] .fieldcontrol", g.element).width(width);
                }
            }
        },

        _setLabel: function (tabindex, fieldindex, label)
        {
            var g = this, p = this.options;
            if (tabindex == null)
            {
                $("li.l-fieldcontainer[fieldindex=" + fieldindex + "] .fieldlabel", g.element).html(label);
            }
            else
            {
                $("div." + p.clsTabContent + "[data-index=" + tabindex + "] li.l-fieldcontainer[fieldindex=" + fieldindex + "] .fieldlabel", g.element).html(label);
            }
        },
        getData: function ()
        {
            ...
            function getFieldValueToData(fields, tabIndex)
            {
                $(fields).each(function (fieldIndex, field)
                {
                    ...
                    if (name)
                    {
                        ...
                        g._setValueByName(data, name, value);
                    }
                    if (textField && editor.editor.getText)
                    {
                        ...
                        g._setValueByName(data, textField, value);
                    }
                });
            }
            return data;
        ...
        _buliderLabelContainer: function (field)
        {
            ...
            out.push(' class="');
            var labelCls = [];
            labelCls.push("fieldlabel");

            if (field.labelCss)
            {
                labelCls.push(field.labelCss);
            }
            else if (p.labelCss)
            {
                labelCls.push(p.labelCss);
            }
            if (field.type == "checkbox")
            {
                labelCls.push("l-label-checkbox");
            }
            out.push(labelCls.join(' '));
        ...
        _buliderControlContainer: function (field, fieldIndex, idPrev)
        {
            ...
            out.push('">');
            if (field.type == "html") //直接输出html内容
            {
                if (field.editor && field.editor.htmlUrl && window.require)
                {
                    require(["text!" + field.editor.htmlUrl], function (content)
                    {
                        setTimeout(function ()
                        {
                            if (field.editor.render)
                            { 
                                field.editor.render.call(g, {
                                    content: content,
                                    renderTo: document.getElementById(fieldId)
                                });
                            }
                            else
                            {
                ...
                else if (field.editor && field.editor.content)
                {
                    var content = field.editor.content;
                    if (field.editor.render)
                    {
                        setTimeout(function ()
                        {
            ...
            if (field.type == "render") //提供当前dom 外部根据需要处理 
            {
                if (field.editor && field.editor.render)
                {
                    setTimeout(function ()
                    {
                        field.editor.render.call(g, {
        ...
        _buliderSpaceContainer: function (field)
        {
            ...
            if (field.spaceContent)
            {
                out.push(field.spaceContent);
            }
            out.push('</li>');
            return out.join('');
        },
        ...
        _setTab: function (tab)
        {
            ...
            for (var i = 0; i < tab.items.length; i++)
            {
                ...
                {
                    jcontentItem.html(tabItem.content);
                }
                else if (tabItem.target)
                {
                    jcontentItem.append(tabItem.target);
                }
            }
            ...
            }).click(function ()
            {
                ...
                if (g.trigger('selectTab', [index]) == false) return;
                g.selectTab(index);
            });
            g.selectTab( 0 );
        ...
        selectTab: function (index)
        {
            ...
            jtab.attr("data-currentindex", index);
        ...
        getCurrentTabIndex: function ()
        {
            var g = this, p = this.options;
            var jtab = $(g.element).find(".l-form-tabs:first");
            return parseInt(jtab.attr("data-currentindex") || 0);
        },
        destroy: function ()
        {
            try
