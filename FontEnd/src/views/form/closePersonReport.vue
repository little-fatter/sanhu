...
<template>
  <div class="wrapper">
    <div class="title">结案报告（个人）</div>
    <div class="content">
      <div class="box">
        <a-row class="row">
          <a-col class="colSize colLine" :span="4">案由：</a-col>
          <a-col class="colSize" :span="12">{{ data.CauseOfAction || '乱丢垃圾' }}</a-col>
        </a-row>
        <a-row class="row">
          <a-col class="colSize colLine" :span="4">当事人:</a-col>
          <a-col class="colSize" :span="12">
            <div>
              <span> {{ data.PartyName || '张伟达' }}  |</span>
              <span> {{ data.PartySex == "f"?'女' : '男' }} |</span>
              <span> {{ data.PartyAge || '26' }} |</span>
              <span> {{ data.PartyJob || '自由职业' }}</span>
            </div>
            <div>
              <span>身份证：</span>
        ...
        <a-row class="row">
          ...
          <a-col class="colSize" :span="12">{{ data.Source || '网上投诉' }}</a-col>
        </a-row>
        <a-row class="row">
...
export default {
  ...
  data () {
    return {
      data: {},
      id: '49d7d99e-49bc-4ab3-854a-4a7eaa475c58'
    }
  },
  mounted () {
  ...
  methods: {
    // 获取结案报告
    getFormReport () {
      getDetails('from_APRPerson', this.id).then(res => {
        this.data = res
