        ...
        <a-col :span="16">
          ...
          <span class="ant-col-20">
            <a-input defaultValue="请输入事发地点" class="ant-col-24" v-model="caseInfo.caseLocation">
              <a-button @click="viewMap" slot="addonAfter" type="primary" shape="circle" icon="environment" size="small"/>
            </a-input>
            <!--    地图查看弹窗-->
          <a-modal
            title="请选择位置"
...
<script>
  export default {
    ...
    mounted() {
      ...
      let date = new Date()
      //此处应该获取后台最后一个案件的编号
      let testCaseNmuber = '20201234' //测试的案件编号
      if (testCaseNmuber == '') {
        this.caseInfo.caseNumber = date.getFullYear() + '1'
      } else {
        let lastNumber = testCaseNmuber.slice(4)//获取除开年份的案件编号
